...
class ReviewController extends Controller
{
    ...
    public function createReview(Request $request, string $slug)
    {
        ...
        if ($validator->fails()) {
            return ErrorController::handleError('Tytuł powinen zawierać między 3 a 30 znaków a opis między 15 a 100 znaków', 400, 'failed');
        }

        $userId = auth()->user()->id;

        $review = Review::where('user_id', '=', $userId)->where('movie_id', '=', $movie->id)->get();

        if (count($review) !== 0) {
            return ErrorController::handleError('Do tego filmu dodałeś już opinię!', 400);
        }
